#Apa coeg mau recod😏
#Silahkan bro yg penting manfaat buat banyak orang ya😁
#Cyber_X404
#Coding By: Pro ciding gila & Chemod_zrd404
daun="\033[32;1m"
tai="\033[33;1m"
indigo="\033[34;1m"
ungu="\033[35;1m"
cyan="\033[36;1m"
white="\033[37;1m"
darah="\033[31;1m"
clear
echo
echo
echo
echo
echo $ungu"================================================================="
echo
toilet -f future "   Tools Installer" | lolcat
echo $daun"                   Coding"$white" By"$darah":"$daun" Chemod_zrd"$darah"404"
echo
echo $ungu"================================================================="
echo $darah"                      Tools Yg Tersedia"
echo "_________________________________________________________________" | lolcat
echo $white"01"$tai"."$cyan" Spamer gabungan" $white"         11"$tai"."$cyan" Fsociety"
echo $white"02"$tai"."$cyan" Bot komena     " $white"         12"$tai"."$cyan" Mps youtube"
echo $white"03"$tai"."$cyan" SpiderBot      " $white"         13"$tai"."$cyan" Gado-Gado"
echo $white"04"$tai"."$cyan" Pemulung BTC   " $white"         14"$tai"."$cyan" Cyber Scan"
echo $white"05"$tai"."$cyan" Termux-Mpv     " $white"         15"$tai"."$cyan" Cok-Rat"
echo $white"06"$tai"."$cyan" Ngrok          " $white"         16"$tai"."$cyan" Insta Bot"
echo $white"07"$tai"."$cyan" Hulk           " $white"         17"$tai"."$cyan" 1337Hash"
echo $white"08"$tai"."$cyan" Auxscan2.0     " $white"         18"$tai"."$cyan" Termux-lazysqlmap"
echo $white"09"$tai"."$cyan" TuyulBtn       " $white"         19"$tai"."$cyan" Nikto"
echo $white"10"$tai"."$cyan" Lazymux        " $white"         20"$tai"."$cyan" Black-Hydra"
echo "_________________________________________________________________" | lolcat
echo $darah"["$white"N"$darah"]"$tai"."$daun" NEXT TOOLS" $darah"       ["$white"I"$darah"]"$tai"."$daun" INFORMASI TOOLS" $darah"       ["$white"E"$darah"]"$tai"."$daun" EXIT"
echo "_________________________________________________________________" | lolcat
echo $white "╭─"$tai"Chemod"$daun"@"$tai"zrd"$darah"404"$cyan" ~/Home 1"$white
read -p " ╰─$ " satu
#ALAT PENDUKUNG
if [ $satu = "I" ] || [ $satu = "i" ]
then
echo
echo
echo
clear
echo
echo
echo
echo
toilet -f future "   Informasi Tools" | lolcat
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo $daun"Nama Tools"$white":"$darah" Tools Installer"
echo $daun"Author    "$white":"$daun " Chemod"$white"_"$daun"zrd"$darah"404"
echo $daun"Gthub     "$white":"$darah" https://github.com/21D4N404"
echo $daun"Blog      "$white":"$darah" http://generasicyberindo.blogspot.com"
echo
echo $darah"                      ("$daun"C"$darah")"$white"_"$tai"2018"$darah" Cyber Famlly"
echo
echo $darah"                       By"$white":"$darah" Subang Cyber Team"
echo
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo "＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿" | lolcat
echo
echo $cyan"["$tai"B"$cyan"]"$white"back "$cyan"["$tai"Q"$cyan"]"$white"Quit"
read -p "[B/Q] " back
fi

if [ $back = "B" ] || [ $back = "b" ]
then
sh jomblo.sh
fi

if [ $back = "Q" ] || [ $back = "q" ]
then
clear
fi

if [ $satu = "E" ] || [ $satu = "e" ]
then
echo $darah" Good Byee..... Jngan lupa share tools ini agar bermanfaat!"
exit
clear
fi

#ISI TOOLS INSTALLER
#HALAMAN KE 1

if [ $satu = 01 ] || [ $satu = 1 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/21D4N404/spamer.git
mv spamer $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 02 ] || [ $satu = 2 ]
then
clear
apt upgrade && apt update -y
git clone https://github.com/Senitopeng/Botkomena.git
mv Botkomena $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 03 ] || [ $satu = 3 ]
then
clear
apt update && apt upgrade -y
git clone https://github.com/Cvar1984/SpiderBot.git
mv SpiderBot $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 04 ] || [ $satu = 4 ]
then
clear
apt update && apt upgrade -y
git clone https://github.com/Cvar1984/pemulungBTC.git
mv pemulungBTC $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 05 ] || [ $satu = 5 ]
then
clear
apt upgrade && apt update -y
git clone https://github.com/Neo-Oli/Termux-Mpv.git
mv Termux-Mpv $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 06 ] || [ $satu = 6 ]
then
clear
apt update && apt upgrade -y
wget https://bin.equinox.io/c/4VmDzA7iaHb/ngrok-stable-linux-arm.zip
mv ngrok-stable-linux-arm.zip $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 07 ] || [ $satu = 7 ]
then
clear
apt update && apt upgrade -y
apt install python2 -y
git clone https://github.com/grafov/hulk.git
mv hulk $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 08 ] || [ $satu = 8 ]
then
clear
apt update && apt upgrade -y
apt install python -y
apt install python2 -y
git clone https://github.com/Gameye98/Auxscan2.0.git
mv Auxscan2.0 $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 09 ] || [ $satu = 9 ]
then
clear
apt update && apt upgrade -y
apt install php -y
git clone https://github.com/Senitopeng/TuyulBtn.git
mv TuyulBtn $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 10 ] || [ $satu = 10 ]
then
clear
apt update && apt upgrade -y
git clone https://github.com/Gameye98/Lazymux.git
mv Lazymux $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

#HALAMAN KE 2

if [ $satu = 11 ] || [ $satu = 11 ]
then
clear
apt update && apt upgrade -y
apt install python2 -y
git clone https://github.com/Manisso/fsociety.git
mv fsociety $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 12 ] || [ $satu = 12 ]
then
clear
apt update && apt upgrade -y
pkg install python -y
pkg install python2 -y
pip install mps_youtube -y
pip install youtube_dl -y
pkg install mpv -y
mv mps_youtube $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 13 ] || [ $satu = 13 ]
then
clear
apt update && apt upgrade -y
pip2 install lolcat -y
git clone https://github.com/Senitopeng/GadoGado.git
mv GadoGado $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 14 ] || [ $satu = 14 ]
then
clear
apt update && apt upgrade -y
git clone https://github.com/medbenali/CyberScan.git
mv CyberScan 
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 15 ] || [ $satu = 15 ]
then
clear
apt update && apt upgrade -y
git clone https://github.com/mrcakil/cok-Rat.git
mv cok-Rat $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 16 ] || [ $satu = 16 ]
then
clear
apt update && apt upgrade -y
git clone https://github.com/instabot-py/instabot.py
mv instabot.py $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 17 ] || [ $satu = 17 ]
then
clear
apt update && apt upgrade -y
apt install python2 -y
git clone https://github.com/Gameye98/1337Hash.git
mv 1337Hash $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 18 ] || [ $satu = 18 ]
then
clear
apt update && apt upgrade -y
git clone https://github.com/verluchie/termux-lazysqlmap.git
mv termux-lazysqlmap $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 19 ] || [ $satu = 19 ]
then
clear
apt update && apt upgrade -y
git clone https://github.com/sullo/nikto.git
mv nikto $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi

if [ $satu = 20 ] || [ $satu = 20 ]
then
clear
apt update && apt upgrade -y
apt install python2 hydra
git clone https://github.com/Gameye98/Black-Hydra.git
mv Black-Hydra $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


#HOME 2


if [ $satu = "N" ] || [ $satu = "n" ]
then

clear
echo
echo
echo
echo
echo $ungu"================================================================="
echo
toilet -f future "   Tools Installer" | lolcat
echo $daun"                   Coding"$white" By"$darah":"$daun" Chemod_zrd"$darah"404"
echo
echo $ungu"================================================================="
echo $darah"                      Tools Yg Tersedia"
echo "_________________________________________________________________" | lolcat
echo $white"21"$tai"."$cyan" BinGoo                  " $white"         31"$tai"."$cyan" Spam mail honeypot"
echo $white"22"$tai"."$cyan" Termux-Archlinux        " $white"         32"$tai"."$cyan" IPGeolocation"
echo $white"23"$tai"."$cyan" Nethunter In Termux     " $white"         33"$tai"."$cyan" Hostchecker"
echo $white"24"$tai"."$cyan" Termux-Ubuntu           " $white"         34"$tai"."$cyan" Poet"
echo $white"25"$tai"."$cyan" Termux-Fedora.sh        " $white"         35"$tai"."$cyan" Shell Checker"
echo $white"26"$tai"."$cyan" Katoolin                " $white"         36"$tai"."$cyan" Fbbrute"
echo $white"27"$tai"."$cyan" Md5-crack               " $white"         37"$tai"."$cyan" Mbf (Multi Brure Force)"
echo $white"28"$tai"."$cyan" Mamangkey               " $white"         38"$tai"."$cyan" Instahack"
echo $white"29"$tai"."$cyan" Myenc                   " $white"         39"$tai"."$cyan" PyBozoCrack"
echo $white"30"$tai"."$cyan" Hashzer                 " $white"         40"$tai"."$cyan" Pybelt"
echo "_________________________________________________________________" | lolcat
echo $darah"["$white"N"$darah"]"$tai"."$daun" NEXT TOOLS" $darah"       ["$white"I"$darah"]"$tai"."$daun" INFORMASI TOOLS" $darah"       ["$white"E"$darah"]"$tai"."$daun" EXIT"
echo "_________________________________________________________________" | lolcat
echo $white "╭─"$tai"Chemod"$daun"@"$tai"zrd"$darah"404"$cyan" ~/Home 2"$white
read -p " ╰─$ " dua

fi


#ALAT PENDUKUNG



if [ $dua = "I" ] || [ $dua = "i" ]
then
echo
echo
echo
clear
echo
echo
echo
echo
toilet -f future "   Informasi Tools" | lolcat
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo $daun"Nama Tools"$white":"$darah" Tools Installer"
echo $daun"Author    "$white":"$daun " Chemod"$white"_"$daun"zrd"$darah"404"
echo $daun"Gthub     "$white":"$darah" https://github.com/21D4N404"
echo $daun"Blog      "$white":"$darah" http://generasicyberindo.blogspot.com"
echo
echo $darah"                      ("$daun"C"$darah")"$white"_"$tai"2018"$darah" Cyber Famlly"
echo
echo $darah"                       By"$white":"$darah" Subang Cyber Team"
echo
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo "＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿" | lolcat
echo
echo $cyan"["$tai"B"$cyan"]"$white"back "$cyan"["$tai"Q"$cyan"]"$white"Quit"
read -p "[B/Q] " back
fi

if [ $back = "B" ] || [ $back = "b" ]
then
sh jomblo.sh
fi

if [ $back = "Q" ] || [ $back = "q" ]
then
clear
fi

if [ $dua = "E" ] || [ $dua = "e" ]
then
echo $darah" Good Byee..... Jngan lupa share tools ini agar bermanfaat! "
exit
clear
fi


#ISI TOOLS INSTALLER
#HALAMAN KE 1


if [ $dua = 21 ] || [ $dua = 21 ]
then
clear
apt update && apt upgrade -y
apt install bash
git clone https://github.com/Hood3dRob1n/BinGoo.git
mv BinGoo $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 22 ] || [ $dua = 22 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
git clone https://github.com/sdrausty/termux-archlinux.git
mv termux-archlinux $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 23 ] || [ $dua = 23 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
git clone https://github.com/Hax4us/Nethunter-In-Termux.git
mv Nethunter-In-Termux $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 24 ] || [ $dua = 24 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
git clone https://github.com/Neo-Oli/termux-ubuntu.git
mv termux-ubuntu $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 25 ] || [ $dua = 25 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
apt install wget
https://raw.githubusercontent.com/nmilosev/termux-fedora/master/termux-fedora.sh
mv termux-fedora.sh $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 26 ] || [ $dua = 26 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
pkg install gnupg
git clone https://github.com/LionSec/katoolin.git
mv katoolin $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 27 ] || [ $dua = 27 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
git clone https://github.com/CiKu370/md5-crack.git
mv md5-crack $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 28 ] || [ $dua = 28 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
git clone https://github.com/Amriez/MamangKey.git
mv MamangKey $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 29 ] || [ $dua = 29 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
git clone https://github.com/pirmansx/myenc.git
mv myenc $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 30 ] || [ $dua = 30 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
pip2 install requests
git clone https://github.com/Anb3rSecID/Hashzer.git
mv Hashzer $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



HALAMAN KE 2



if [ $dua = 31 ] || [ $dua = 31 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
pkg install php
git clone https://github.com/whackashoe/php-spam-mail-honeypot.git
mv php-spam-mail-honeypot $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 32 ] || [ $dua = 32 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
git clone https://github.com/maldevel/IPGeolocation.git
mv IPGeolocation $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 33 ] || [ $dua = 33 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
git clone https://github.com/pirmansx/hostchecker.git
mv hostchecker $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 34 ] || [ $dua = 34 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
git clone https://github.com/mossberg/poet.git
mv poet $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 35 ] || [ $dua = 35 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
wget http://pastebin.com/raw/Y0cqkjrj --output-document=ch01.py
mkdir shell ~
mv ch01.py ~/shell $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 36 ] || [ $dua = 36 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
pip2 install mechanize
apt install git
git clone https://github.com/Senitopeng/fbbrute.git
mv fbbrute $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 37 ] || [ $dua = 37 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
git clone https://github.com/pirmansx/mbf.git
mv mbf $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 38 ] || [ $dua = 38 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
pip2 install requests
git clone https://github.com/avramit/instahack.git
mv instahack $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 39 ] || [ $dua = 39 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
pip2 install bozocrack
git clone https://github.com/ikkebr/PyBozoCrack.git
mv PyBozoCrack $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $dua = 40 ] || [ $dua = 40 ]
then
clear
apt update && apt upgrade -y
apt install bash
apt install python2
apt install git
git clone https://github.com/ekultek/pybelt.git
pip install -r requirements.txt
mv pybelt $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




#HOME BARU!!!



if [ $dua = "N" ] || [ $dua = "n" ]
then

clear
echo
echo
echo
echo
echo $ungu"================================================================="
echo
toilet -f future "   Tools Installer" | lolcat
echo $daun"                   Coding"$white" By"$darah":"$daun" Chemod_zrd"$darah"404"
echo
echo $ungu"================================================================="
echo $darah"                      Tools Yg Tersedia"
echo "_________________________________________________________________" | lolcat
echo $white"41"$tai"."$cyan" EvilURL             " $white"         51"$tai"."$cyan" AngryFuzzer"
echo $white"42"$tai"."$cyan" Credmap             " $white"         52"$tai"."$cyan" Toolss"
echo $white"43"$tai"."$cyan" Netattack           " $white"         53"$tai"."$cyan" Termux-go"
echo $white"44"$tai"."$cyan" IesInstall          " $white"         54"$tai"."$cyan" Bolang"
echo $white"45"$tai"."$cyan" B4J1N64Nv5          " $white"         55"$tai"."$cyan" XPL-SEARCH"
echo $white"46"$tai"."$cyan" B4J1N64Nv6          " $white"         56"$tai"."$cyan" Webpwn3r"
echo $white"47"$tai"."$cyan" Diejoubu            " $white"         57"$tai"."$cyan" Ffont"
echo $white"48"$tai"."$cyan" Error cyber tool    " $white"         58"$tai"."$cyan" MeTAInstall"
echo $white"49"$tai"."$cyan" MR.Cakil            " $white"         59"$tai"."$cyan" IPscan"
echo $white"50"$tai"."$cyan" Hack tronian        " $white"         60"$tai"."$cyan" NetKiller"
echo "_________________________________________________________________" | lolcat
echo $darah"["$white"N"$darah"]"$tai"."$daun" NEXT TOOLS" $darah"       ["$white"I"$darah"]"$tai"."$daun" INFORMASI TOOLS" $darah"       ["$white"E"$darah"]"$tai"."$daun" EXIT"
echo "_________________________________________________________________" | lolcat
echo $white "╭─"$tai"Chemod"$daun"@"$tai"zrd"$darah"404"$cyan" ~/Home 3"$white
read -p " ╰─$ " tiga

fi




#ALAT PENDUKUNG



if [ $tiga = "I" ] || [ $tiga = "i" ]
then
echo
echo
echo
clear
echo
echo
echo
echo
toilet -f future "   Informasi Tools" | lolcat
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo $daun"Nama Tools"$white":"$darah" Tools Installer"
echo $daun"Author    "$white":"$daun " Chemod"$white"_"$daun"zrd"$darah"404"
echo $daun"Gthub     "$white":"$darah" https://github.com/21D4N404"
echo $daun"Blog      "$white":"$darah" http://generasicyberindo.blogspot.com"
echo
echo $darah"                      ("$daun"C"$darah")"$white"_"$tai"2018"$darah" Cyber Famlly"
echo
echo $darah"                       By"$white":"$darah" Subang Cyber Team"
echo
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo "＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿" | lolcat
echo
echo $cyan"["$tai"B"$cyan"]"$white"back "$cyan"["$tai"Q"$cyan"]"$white"Quit"
read -p "[B/Q] " back
fi

if [ $back = "B" ] || [ $back = "b" ]
then
sh jomblo.sh
fi

if [ $back = "Q" ] || [ $back = "q" ]
then
clear
fi

if [ $tiga = "E" ] || [ $tiga = "e" ]
then
echo $darah"Good Byee..... Jngan lupa share tools ini agar bermanfaat! "
exit
clear
fi


#ISI TOOLS INSTALLER
#BAGIAN 1



if [ $tiga = 41 ] || [ $tiga = 41 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/UndeadSec/EvilURL.git
mv EvilURL $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $tiga = 42 ] || [ $tiga = 42 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone clone https://github.com/lightos/credmap.git
mv credmap $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $tiga = 43 ] || [ $tiga = 43 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/chrizator/netattack.git
mv netattack $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $tiga = 44 ] || [ $tiga = 44 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/ALX-04/iesInstall
mv iesInstall $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $tiga = 45 ] || [ $tiga = 45 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install toilet -y
pkg install figlet -y
git clone https://github.com/DarknessCyberTeam/B4J1N64Nv5.git
mv B4J1N64Nv5 $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $tiga = 46 ] || [ $tiga = 46 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install toilet -y
pkg install figlet -y
git clone https://github.com/DarknessCyberTeam/BAJINGANv6.git
mv B4J1N64Nv6 $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi


if [ $tiga = 47 ] || [ $tiga = 47 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/alintamvanz/diejoubu.git
mv diejoubu $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tiga = 48 ] || [ $tiga = 48 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/MrKeepSmile/errorcybertool.git
mv errorcybertool $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tiga = 49 ] || [ $tiga = 49 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/mrcakil/Mrcakil
mv Mrcakil $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tiga = 50 ] || [ $tiga = 50 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/thehackingsage/hacktronian.git
mv hacktronian $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi





#BAGIAN 2






if [ $tiga = 51 ] || [ $tiga = 51 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/ihebski/angryFuzzer.git
pip2 install -r requirements
pip2 install requests
mv angryFuzzer $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tiga = 52 ] || [ $tiga = 52 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/AnonHackerr/toolss.git
mv toolss $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tiga = 53 ] || [ $tiga = 53 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/rafalgolarz/termux-go.git
mv termux-go $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tiga = 54 ] || [ $tiga = 54 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/Amriez/Bolang.git
mv Bolang $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tiga = 55 ] || [ $tiga = 55 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/r00tmars/XPL-SEARCH.git
mv XPL-SEARCH $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tiga = 56 ] || [ $tiga = 56 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/zigoo0/webpwn3r.git
mv webpwn3r $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tiga = 57 ] || [ $tiga = 57 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/4L13199/ffont.git
mv ffont $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tiga = 58 ] || [ $tiga = 58 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/4L13199/meTAInstall.git
mv meTAInstall $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tiga = 59 ] || [ $tiga = 59 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/sysadminteam/IPscan.git
mv IPscan $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tiga = 60 ] || [ $tiga = 60 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/sysadminteam/NetKiller.git
mv NetKiller $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



#HOME BARU



if [ $tiga = "N" ] || [ $tiga = "n" ]
then

clear
echo
echo
echo
echo
echo $ungu"================================================================="
echo
toilet -f future "   Tools Installer" | lolcat
echo $daun"                   Coding"$white" By"$darah":"$daun" Chemod_zrd"$darah"404"
echo
echo $ungu"================================================================="
echo $darah"                      Tools Yg Tersedia"
echo "_________________________________________________________________" | lolcat
echo $white"61"$tai"."$cyan" Spammer sys         " $white"         71"$tai"."$cyan" Sqliv"
echo $white"62"$tai"."$cyan" OSIF                " $white"         72"$tai"."$cyan" Easymap"
echo $white"63"$tai"."$cyan" Infoga              " $white"         73"$tai"."$cyan" Crips"
echo $white"64"$tai"."$cyan" Andro Zenmap        " $white"         74"$tai"."$cyan" Sir"
echo $white"65"$tai"."$cyan" Astra Nmap          " $white"         75"$tai"."$cyan" Xshell"
echo $white"66"$tai"."$cyan" WTF                 " $white"         76"$tai"."$cyan" Sqldump"
echo $white"67"$tai"."$cyan" Black Boox          " $white"         77"$tai"."$cyan" Sqlokmed"
echo $white"68"$tai"."$cyan" WPSeku              " $white"         78"$tai"."$cyan" Zones"
echo $white"69"$tai"."$cyan" Cupp                " $white"         79"$tai"."$cyan" SH33LL"
echo $white"70"$tai"."$cyan" AndroidPINCrack     " $white"         80"$tai"."$cyan" Metasploit"
echo "_________________________________________________________________" | lolcat
echo $darah"["$white"N"$darah"]"$tai"."$daun" NEXT TOOLS" $darah"       ["$white"I"$darah"]"$tai"."$daun" INFORMASI TOOLS" $darah"       ["$white"E"$darah"]"$tai"."$daun" EXIT"
echo "_________________________________________________________________" | lolcat
echo $white "╭─"$tai"Chemod"$daun"@"$tai"zrd"$darah"404"$cyan" ~/Home 4"$white
read -p " ╰─$ " empat

fi



#ALAT PENDUKUNG



if [ $empat = "I" ] || [ $empat = "i" ]
then
echo
echo
echo
clear
echo
echo
echo
echo
toilet -f future "   Informasi Tools" | lolcat
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo $daun"Nama Tools"$white":"$darah" Tools Installer"
echo $daun"Author    "$white":"$daun " Chemod"$white"_"$daun"zrd"$darah"404"
echo $daun"Gthub     "$white":"$darah" https://github.com/21D4N404"
echo $daun"Blog      "$white":"$darah" http://generasicyberindo.blogspot.com"
echo
echo $darah"                      ("$daun"C"$darah")"$white"_"$tai"2018"$darah" Cyber Famlly"
echo
echo $darah"                       By"$white":"$darah" Subang Cyber Team"
echo
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo "＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿" | lolcat
echo
echo $cyan"["$tai"B"$cyan"]"$white"back "$cyan"["$tai"Q"$cyan"]"$white"Quit"
read -p "[B/Q] " back
fi

if [ $back = "B" ] || [ $back = "b" ]
then
sh jomblo.sh
fi

if [ $back = "Q" ] || [ $back = "q" ]
then
clear
fi

if [ $empat = "E" ] || [ $empat = "e" ]
then
echo $darah"Good Byee..... Jngan lupa share tools ini agar bermanfaat! "
exit
clear
fi





#ISI TOOLS INSTALLER
#Halaman 1



if [ $empat = 61 ] || [ $empat = 61 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/sysadminteam/spammer.sys
mv spammer.sys $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $empat = 62 ] || [ $empat = 62 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone clone https://github.com/ciku370/OSIF.git
mv OSIF $HOME
cd ~/OSIF
pip2 install -r requirements.txt
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $empat = 63 ] || [ $empat = 63 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pip2 install requests urllib3 urlparse
git clone https://github.com/m4ll0k/Infoga.git
mv Infoga $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $empat = 64 ] || [ $empat = 64 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install nmap curl
curl -O http://override.waper.co/files/androzenmap.txt
mkdir ~/AndroZenmap
mv androzenmap.txt ~/AndroZenmap/androzenmap.sh
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $empat = 65 ] || [ $empat = 65 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install git nmap
git clone https://github.com/Gameye98/AstraNmap.git
mv AstraNmap $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $empat = 66 ] || [ $empat = 66 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pip2 bs4 requests HTMLParser urlparse mechanize argparse
git clone https://github.com/Xi4u7/wtf.git
mv wtf $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $empat = 67 ] || [ $empat = 67 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install python2 git && pip2 install optparse passlib
git clone https://github.com/jothatron/blackbox.git
mv blackbox $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $empat = 68 ] || [ $empat = 68 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/m4ll0k/WPSeku.git
mv WPSeku ~
cd ~/WPSeku
pip2 install -r requirements.txt
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $empat = 69 ] || [ $empat = 69 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/Mebus/cupp.git
mv cupp $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $empat = 70 ] || [ $empat = 70 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/PentesterES/AndroidPINCrack.git
mv AndroidPINCrack $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi





#HALAMAN 2





if [ $empat = 71 ] || [ $empat = 71 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/Hadesy2k/sqliv.git
mv sqliv $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $empat = 72 ] || [ $empat = 72 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/Cvar1984/Easymap.git
mv Easymap ~
cd ~/Easymap && sh install.sh
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $empat = 73 ] || [ $empat = 73 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pip2 install bs4 urllib2
apt install python2 openssl curl libcurl wget
git clone https://github.com/Manisso/Crips.git
mv sir $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $empat = 74 ] || [ $empat = 74 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pip2 install bs4 urllib2
git clone https://github.com/AeonDave/sir.git
mv sir $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $empat = 75 ] || [ $empat = 75 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install lynx python2 figlet ruby php nano w3m
git clone https://github.com/Ubaii/Xshell.git
mv Xshell $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $empat = 76 ] || [ $empat = 76 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pip2 install google
curl -k -O https://gist.githubusercontent.com/Gameye98/76076c9a282a6f32749894d5368024a6/raw/6f9e754f2f81ab2b8efda30603dc8306c65bd651/sqldump.py
mkdir ~/sqldump && chmod +x sqldump.py && mv sqldump.py ~/sqldump
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $empat = 77 ] || [ $empat = 77 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pip2 install urllib2
git clone https://github.com/Anb3rSecID/sqlokmed.git
mv sqlokmed $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $empat = 78 ] || [ $empat = 78 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install php
git clone https://github.com/Cvar1984/zones.git
mv zones $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $empat = 79 ] || [ $empat = 79 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/LOoLzeC/SH33LL.git
mv SH33LL $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $empat = 80 ] || [ $empat = 80 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install curl
curl -LO https://raw.githubusercontent.com/Hax4us/Metasploit_termux/master/metasploit.sh
chmod +x metasploit.sh
mv metasploit.sh ~
cd ~ && ./metasploit.sh
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi





#HOME BARU





if [ $empat = "N" ] || [ $empat = "n" ]
then

clear
echo
echo
echo
echo
echo $ungu"================================================================="
echo
toilet -f future "   Tools Installer" | lolcat
echo $daun"                   Coding"$white" By"$darah":"$daun" Chemod_zrd"$darah"404"
echo
echo $ungu"================================================================="
echo $darah"                      Tools Yg Tersedia"
echo "_________________________________________________________________" | lolcat
echo $white"81"$tai"."$cyan" A-Rat             " $white"         91"$tai"."$cyan" Cred Sniper"
echo $white"82"$tai"."$cyan" The Fat Rat       " $white"         92"$tai"."$cyan" Pyphisher"
echo $white"83"$tai"."$cyan" Brutal            " $white"         93"$tai"."$cyan" Weeman"
echo $white"84"$tai"."$cyan" Routersploit      " $white"         94"$tai"."$cyan" Phising Game"
echo $white"85"$tai"."$cyan" Xgans             " $white"         95"$tai"."$cyan" Termux-ohmyzsh"
echo $white"86"$tai"."$cyan" Wifite            " $white"         96"$tai"."$cyan" Termux-ohmyfish"
echo $white"87"$tai"."$cyan" Wifiphisher       " $white"         97"$tai"."$cyan" Social Fish"
echo $white"88"$tai"."$cyan" Wifi Hacker       " $white"         98"$tai"."$cyan" LITESPAM"
echo $white"89"$tai"."$cyan" Pixiewps          " $white"         99"$tai"."$cyan" ANRspam"
echo $white"90"$tai"."$cyan" BluePot-0.1       " $white"        100"$tai"."$cyan" Gcospam"
echo "_________________________________________________________________" | lolcat
echo $darah"["$white"N"$darah"]"$tai"."$daun" NEXT TOOLS" $darah"       ["$white"I"$darah"]"$tai"."$daun" INFORMASI TOOLS" $darah"       ["$white"E"$darah"]"$tai"."$daun" EXIT"
echo "_________________________________________________________________" | lolcat
echo $white "╭─"$tai"Chemod"$daun"@"$tai"zrd"$darah"404"$cyan" ~/Home 5"$white
read -p " ╰─$ " lima

fi



#ALAT PENDUKUNG



if [ $lima = "I" ] || [ $lima = "i" ]
then
echo
echo
echo
clear
echo
echo
echo
echo
toilet -f future "   Informasi Tools" | lolcat
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo $daun"Nama Tools"$white":"$darah" Tools Installer"
echo $daun"Author    "$white":"$daun " Chemod"$white"_"$daun"zrd"$darah"404"
echo $daun"Gthub     "$white":"$darah" https://github.com/21D4N404"
echo $daun"Blog      "$white":"$darah" http://generasicyberindo.blogspot.com"
echo
echo $darah"                      ("$daun"C"$darah")"$white"_"$tai"2018"$darah" Cyber Famlly"
echo
echo $darah"                       By"$white":"$darah" Subang Cyber Team"
echo
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo "＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿" | lolcat
echo
echo $cyan"["$tai"B"$cyan"]"$white"back "$cyan"["$tai"Q"$cyan"]"$white"Quit"
read -p "[B/Q] " back
fi

if [ $back = "B" ] || [ $back = "b" ]
then
sh jomblo.sh
fi

if [ $back = "Q" ] || [ $back = "q" ]
then
clear
fi

if [ $lima = "E" ] || [ $lima = "e" ]
then
echo $darah"Good Byee..... Jngan lupa share tools ini agar bermanfaat! "
exit
clear
fi






#ISI TOOLS INSTALLER






if [ $lima = 81 ] || [ $lima = 81 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/Xi4u7/A-Rat.git
mv A-Rat $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $lima = 82 ] || [ $lima = 82 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git git clone https://github.com/Screetsec/TheFatRat.git
mv TheFatRat ~
cd ~/TheFatRat
chmod +x setup.sh && ./setup.sh
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $lima = 83 ] || [ $lima = 83 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/Screetsec/Brutal.git
mv Brutal $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $lima = 84 ] || [ $lima = 84 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pip2 install requests
git clone https://github.com/reverse-shell/routersploit.git
mv routersploit ~;cd ~/routersploit;pip2 install -r requirements.txt;termux-fix-shebang rsf.py
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $lima = 85 ] || [ $lima = 85 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
curl -O http://override.waper.co/files/xgans.txt
mkdir ~/xGans
mv xgans.txt ~/xGans/xgans.py
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $lima = 86 ] || [ $lima = 86 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/derv82/wifite.git
mv wifite $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $lima = 87 ] || [ $lima = 87 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
python-pip python- setuptools
apt-get install python python-pip python-setuptools
git clone https://github.com/wifiphisher/wifiphisher.git
mv wifiphisher ~
cd ~/wifiphisher
python setup.py install
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $lima = 88 ] || [ $lima = 88 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/esc0rtd3w/wifi-hacker.git
mv wifi-hacker ~
cd ~/wifi-hacker
chmod +x wifi-hacker.sh
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $lima = 89 ] || [ $lima = 89 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/wiire/pixiewps.git
mv pixiewps $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $lima = 90 ] || [ $lima = 90 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
wget https://github.com/andrewmichaelsmith/bluepot/raw/master/bin/bluepot-0.1.tar.gz && tar xfz bluepot-0.1.tar.gz && sudo java -jar bluepot/BluePot-0.1.jar
mv bluepot/BluePot-0.1 $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi





#BAGIAN 2






if [ $lima = 91 ] || [ $lima = 91 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/ustayready/CredSniper.git
mv CredSniper ~
cd ~/CredSniper && ./install.sh
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $lima = 92 ] || [ $lima = 92 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install wget
wget http://pastebin.com/raw/DDVqWp4Z --output-document=pisher.py
mkdir pyphisher
mv phisher.py pyphisher
mv pyphisher
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $lima = 93 ] || [ $lima = 93 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pip2 install beautifulsoup bs4
git clone https://github.com/samyoyo/weeman.git
mv weeman $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $lima = 94 ] || [ $lima = 94 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/senitopeng/PhisingGame.git
mv PhisingGame $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $lima = 95 ] || [ $lima = 95 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/Cabbagec/termux-ohmyzsh.git
mv termux-ohmyzsh $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $lima = 96 ] || [ $lima = 96 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install fish
fish
chsh -s fish
apt install curl
curl -L https://get.oh-my.fish > install
fish install --path=~./local/share/omf --config=~/.config/omf
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $lima = 97 ] || [ $lima = 97 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install python2 git && pip2 install wget
git clone https://github.com/UndeadSec/SocialFish.git
mv SocialFish $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $lima = 98 ] || [ $lima = 98 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install toilet php ruby
gem install lolcat
git clone https://github.com/4L13199/LITESPAM.git
mv LITESPAM $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $lima = 99 ] || [ $lima = 99 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install php figlet
git clone https://github.com/Amriez/ANRspam.git
mv ANRspam $HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $lima = 100 ] || [ $lima = 100 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install php figlet
git clone https://github.com/Amriez/gcospam.git
mv gcospam ~
cd ~/gcospam && sh install.sh
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi







#HOME BARU




if [ $lima = "N" ] || [ $lima = "n" ]
then

clear
echo
echo
echo
echo
echo $ungu"================================================================="
echo
toilet -f future "   Tools Installer" | lolcat
echo $daun"                   Coding"$white" By"$darah":"$daun" Chemod_zrd"$darah"404"
echo
echo $ungu"================================================================="
echo $darah"                      Tools Yg Tersedia"
echo "_________________________________________________________________" | lolcat
echo $white"101"$tai"."$cyan" SpazSMS           " $white"         111"$tai"."$cyan" RED_HAWK"
echo $white"102"$tai"."$cyan" AVARspam          " $white"         112"$tai"."$cyan" Devploit"
echo $white"103"$tai"."$cyan" Bom-mail          " $white"         113"$tai"."$cyan" ReconDog"
echo $white"104"$tai"."$cyan" Spammer Grab      " $white"         114"$tai"."$cyan" Sqlmap"
echo $white"105"$tai"."$cyan" Airgeddon         " $white"         115"$tai"."$cyan" Sqlmate"
echo $white"106"$tai"."$cyan" Genscript         " $white"         116"$tai"."$cyan" Sqlscan"
echo $white"107"$tai"."$cyan" SC creator        " $white"         117"$tai"."$cyan" ViSQL"
echo $white"108"$tai"."$cyan" D-TECT            " $white"         118"$tai"."$cyan" AOCDEFACE"
echo $white"109"$tai"."$cyan" Ipmux             " $white"         119"$tai"."$cyan" Webdav"
echo $white"110"$tai"."$cyan" Inmux             " $white"         120"$tai"."$cyan" Wbdav mas ploit"
echo "_________________________________________________________________" | lolcat
echo $darah"["$white"N"$darah"]"$tai"."$daun" NEXT TOOLS" $darah"       ["$white"I"$darah"]"$tai"."$daun" INFORMASI TOOLS" $darah"       ["$white"E"$darah"]"$tai"."$daun" EXIT"
echo "_________________________________________________________________" | lolcat
echo $white "╭─"$tai"Chemod"$daun"@"$tai"zrd"$darah"404"$cyan" ~/Home 6"$white
read -p " ╰─$ " enam

fi



#ALAT PENDUKUNG



if [ $enam = "I" ] || [ $enam = "i" ]
then
echo
echo
echo
clear
echo
echo
echo
echo
toilet -f future "   Informasi Tools" | lolcat
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo $daun"Nama Tools"$white":"$darah" Tools Installer"
echo $daun"Author    "$white":"$daun " Chemod"$white"_"$daun"zrd"$darah"404"
echo $daun"Gthub     "$white":"$darah" https://github.com/21D4N404"
echo $daun"Blog      "$white":"$darah" http://generasicyberindo.blogspot.com"
echo
echo $darah"                      ("$daun"C"$darah")"$white"_"$tai"2018"$darah" Cyber Famlly"
echo
echo $darah"                       By"$white":"$darah" Subang Cyber Team"
echo
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo "＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿" | lolcat
echo
echo $cyan"["$tai"B"$cyan"]"$white"back "$cyan"["$tai"Q"$cyan"]"$white"Quit"
read -p "[B/Q] " back
fi

if [ $back = "B" ] || [ $back = "b" ]
then
sh jomblo.sh
fi

if [ $back = "Q" ] || [ $back = "q" ]
then
clear
fi

if [ $enam = "E" ] || [ $enam = "e" ]
then
echo $darah"Good Byee..... Jngan lupa share tools ini agar bermanfaat! "
exit
clear
fi






#ISI TOOLS BAGIAN 1






if [ $enam = 101 ] || [ $enam = 101 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install php figlet
apt install git python2 && pip2 install requests
git clone https://github.com/Gameye98/SpazSMS.git
mv SpazSMS #HOME
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $enam = 102 ] || [ $enam = 102 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/ALX-04/AVARspam.git
mv AVARspam ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 103 ] || [ $enam = 103 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install php figlet
git clone https://github.com/root-ID-041/bom-mail.git
mv bom-mail
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $enam = 104 ] || [ $enam = 104 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install php figlet
pip2 install requests
git clone https://github.com/p4kl0nc4t/spammer-grab.git
mv spammer-grab
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 105 ] || [ $enam = 105 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install php figlet
git clone https://github.com/v1s1t0r1sh3r3/airgeddon.git
mv airgeddon ~
cd ~/airgeddon
chmod +x airgeddon.sh
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 106 ] || [ $enam = 106 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install perl
git clone https://github.com/x-xsystm/genscript.git
mv genscript ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 107 ] || [ $enam = 107 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/Ubaii/script-deface-cretor.git
mv script-deface-creator ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 108 ] || [ $enam = 108 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/shawarkhanethicalhacker/D-TECT.git
mv D-TECT ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 109 ] || [ $enam = 109 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install w3m wget
git clone https://github.com/Amriez/ipmux.git
mv ipmux ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 110 ] || [ $enam = 110 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install wget w3m
git clone https://github.com/Amriez/inmux.git
mv inmux ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi





#ISI TOOLS BAGIAN 2





if [ $enam = 111 ] || [ $enam = 111 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install php
git clone https://github.com/Tuhinshubhra/RED_HAWK.git
mv RED_HAWK ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 112 ] || [ $enam = 112 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/joker25000/Devploit.git
mv Devploit ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 113 ] || [ $enam = 113 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/UltimateHackers/ReconDog.git
mv ReconDog ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 114 ] || [ $enam = 114 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/sqlmapproject/sqlmap.git
mv sqlmap ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 115 ] || [ $enam = 115 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pip2 install mechanize bs4 HTMLparser argparse requests urlparse2
git clone https://github.com/UltimateHackers/sqlmate.git
mv sqlmate ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 116 ] || [ $enam = 116 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install php
git clone http://www.github.com/Cvar1984/sqlscan.git
mv sqlscan ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 117 ] || [ $enam = 117 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/blackvkng/viSQL.git
mv viSQL ~
cd ~/viSQL
chmod +x 777 viSQL.py
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 118 ] || [ $enam = 118 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install curl python2
git clone https://github.com/Amriez/AOCDEFACE.git
mv AOCDEFACE ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 119 ] || [ $enam = 119 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install python2 openssl curl libcurl
mkdir ~/webdav
curl -k -O http://override.waper.co/files/webdav.txt;mv webdav.txt ~/webdav/webdav.py
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $enam = 120 ] || [ $enam = 120 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install python2 openssl curl libcurl
pip2 install requests
curl -k -O https://pastebin.com/raw/K1VYVHxX && mv K1VYVHxX webdav.py
mkdir ~/webdav-mass-exploit && mv webdav.py ~/webdav-mass-exploit
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi








#HOME BARU










if [ $enam = "N" ] || [ $enam = "n" ]
then

clear
echo
echo
echo
echo
echo $ungu"================================================================="
echo
toilet -f future "   Tools Installer" | lolcat
echo $daun"                   Coding"$white" By"$darah":"$daun" Chemod_zrd"$darah"404"
echo
echo $ungu"================================================================="
echo $darah"                      Tools Yg Tersedia"
echo "_________________________________________________________________" | lolcat
echo $white"121"$tai"."$cyan" Soceng toolkit        " $white"         131"$tai"."$cyan" Torshammer"
echo $white"122"$tai"."$cyan" SCANNER-INURLBR       " $white"         132"$tai"."$cyan" Kawai-Botnet"
echo $white"123"$tai"."$cyan" Ko Dork               " $white"         133"$tai"."$cyan" lhst"
echo $white"124"$tai"."$cyan" V3n0M-Scanner         " $white"         134"$tai"."$cyan" Joomscan"
echo $white"125"$tai"."$cyan" Th3inspector          " $white"         135"$tai"."$cyan" Wpbf"
echo $white"126"$tai"."$cyan" Wfdroid               " $white"         136"$tai"."$cyan" CMSmap"
echo $white"127"$tai"."$cyan" Green Reaper          " $white"         137"$tai"."$cyan" Elfinder"
echo $white"128"$tai"."$cyan" Xerxes                " $white"         138"$tai"."$cyan" Lokomedia"
echo $white"129"$tai"."$cyan" Hammer                " $white"         139"$tai"."$cyan" Dorker"
echo $white"130"$tai"."$cyan" LITEDDOS              " $white"         140"$tai"."$cyan" Com-fabrik"
echo "_________________________________________________________________" | lolcat
echo $darah"["$white"N"$darah"]"$tai"."$daun" NEXT TOOLS" $darah"       ["$white"I"$darah"]"$tai"."$daun" INFORMASI TOOLS" $darah"       ["$white"E"$darah"]"$tai"."$daun" EXIT"
echo "_________________________________________________________________" | lolcat
echo $white "╭─"$tai"Chemod"$daun"@"$tai"zrd"$darah"404"$cyan" ~/Home 7"$white
read -p " ╰─$ " tujuh

fi



#ALAT PENDUKUNG



if [ $tujuh = "I" ] || [ $tujuh = "i" ]
then
echo
echo
echo
clear
echo
echo
echo
echo
toilet -f future "   Informasi Tools" | lolcat
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo $daun"Nama Tools"$white":"$darah" Tools Installer"
echo $daun"Author    "$white":"$daun " Chemod"$white"_"$daun"zrd"$darah"404"
echo $daun"Gthub     "$white":"$darah" https://github.com/21D4N404"
echo $daun"Blog      "$white":"$darah" http://generasicyberindo.blogspot.com"
echo
echo $darah"                      ("$daun"C"$darah")"$white"_"$tai"2018"$darah" Cyber Famlly"
echo
echo $darah"                       By"$white":"$darah" Subang Cyber Team"
echo
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo "＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿" | lolcat
echo
echo $cyan"["$tai"B"$cyan"]"$white"back "$cyan"["$tai"Q"$cyan"]"$white"Quit"
read -p "[B/Q] " back
fi

if [ $back = "B" ] || [ $back = "b" ]
then
sh jomblo.sh
fi

if [ $back = "Q" ] || [ $back = "q" ]
then
clear
fi

if [ $tujuh = "E" ] || [ $tujuh = "e" ]
then
echo $darah"Good Byee..... Jngan lupa share tools ini agar bermanfaat! "
exit
clear
fi





#ISI TOOLS INSTALLER BAGIAN 1





if [ $tujuh = 121 ] || [ $tujuh = 121 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install curl python2
git clone https://github.com/trustedsec/social-engineer-toolkit.git
mv social-engineer-toolkit ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tujuh = 122 ] || [ $tujuh = 122 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install curl python2
apt-get install curl libcurl3 libcurl3-dev php5 php5-cli php5-curl
git clone https://github.com/googleinurl/SCANNER-INURLBR.git
mv SCANNER-INURLBR ~
cd ~/SCANNER-INURLBR ~
chmod +x inurlbr.php
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tujuh = 123 ] || [ $tujuh = 123 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install curl python2
git clone https://github.com/ciku370/ko-dork.git
mv ko-dork ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tujuh = 124 ] || [ $tujuh = 124 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pip2 install dnspython3 aiohttp httplib2 socksipy-branch requests url bs4
git clone https://github.com/v3n0m-Scanner/V3n0M-Scanner
mv V3n0M-Scanner ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tujuh = 125 ] || [ $tujuh = 125 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install perl
git clone https://github.com/Moham3dRiahi/Th3inspector.git
mv Th3inspector ~
cd ~/The3inspector
chmod +x install.sh && ./install.sh
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tujuh = 126 ] || [ $tujuh = 126 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install wget
cd ~ && mkdir wfdroid
wget https://raw.githubusercontent.com/bytezcrew/wfdroid-termux/master/wfdinstall
chmod +x wfdinstall
mv wfdinstall ~/wfdroid
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tujuh = 127 ] || [ $tujuh = 127 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/Amriez/GreenReaper.git
mv GreenReaper ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tujuh = 128 ] || [ $tujuh = 128 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install clang
git clone https://github.com/zanyarjamal/xerxes.git
mv xerxes ~
cd ~/xerxes
clang xerxes.c -o xerxes
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tujuh = 129 ] || [ $tujuh = 129 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/cyweb/hammer.git
mv hammer ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tujuh = 130 ] || [ $tujuh = 130 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/4L13199/LITEDDOS.git
mv LITEDDOS ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi






#ISI TOOLS BAGIAN 2





if [ $tujuh = 131 ] || [ $tujuh = 131 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/dotfighter/torshammer.git
mv torshammer ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $tujuh = 132 ] || [ $tujuh = 132 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install php
git clone https://github.com/Cvar1984/Kawai-Botnet.git
mv Kawai-Botnet ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $tujuh = 133 ] || [ $tujuh = 133 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/CiKu370/lhst.git
mv lhst ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $tujuh = 134 ] || [ $tujuh = 134 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install perl
git clone https://github.com/rezasp/joomscan.git
mv joomscan ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $tujuh = 135 ] || [ $tujuh = 135 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pip2 install requests
git clone https://github.com/atarantini/wpbf.git
mv wpbf ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $tujuh = 136 ] || [ $tujuh = 136 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/Dionach/CMSmap.git
mv CMSmap ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $tujuh = 137 ] || [ $tujuh = 137 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install wget
cd ~ && mkdir elfinder
wget https://pastebin.com/raw/S7Y2V19h -O elfinder.php
mv elfinder.php ~/elfinder
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $tujuh = 138 ] || [ $tujuh = 138 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt-get install php
mkdir lokomed
cd ~/lokomed
wget https://pastebin.com/raw/sPpJRjCZ -O lokomedia.php
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $tujuh = 139 ] || [ $tujuh = 139 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt-get install php
mkdir bing
cd ~/bing
wget https://pastebin.com/raw/tjQY6Tsg -O dorker.php
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $tujuh = 140 ] || [ $tujuh = 140 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt-get install wget
apt-get install php
wget https://pastebin.com/raw/LDvFvtUD -O comfabrik.php
mkdir ~/com-fabrik
mv comfabrik.php ~/com-fabrik
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi







#TAB BARU






if [ $tujuh = "N" ] || [ $tujuh = "n" ]
then

clear
echo
echo
echo
echo
echo $ungu"================================================================="
echo
toilet -f future "   Tools Installer" | lolcat
echo $daun"                   Coding"$white" By"$darah":"$daun" Chemod_zrd"$darah"404"
echo
echo $ungu"================================================================="
echo $darah"                      Tools Yg Tersedia"
echo "_________________________________________________________________" | lolcat
echo $white"141"$tai"."$cyan" Defacer           " $white"         151"$tai"."$cyan" Shellnoob"
echo $white"142"$tai"."$cyan" Hash-Buster       " $white"         152"$tai"."$cyan" Websploit"
echo $white"143"$tai"."$cyan" Breacher          " $white"         153"$tai"."$cyan" XAttacker"
echo $white"144"$tai"."$cyan" OWScan            " $white"         154"$tai"."$cyan" Txtools"
echo $white"145"$tai"."$cyan" Wpscan            " $white"         155"$tai"."$cyan" IFC"
echo $white"146"$tai"."$cyan" Wordpresscan      " $white"         156"$tai"."$cyan" Ro0T_N3T"
echo $white"147"$tai"."$cyan" Ezsploit          " $white"         157"$tai"."$cyan" ACT"
echo $white"148"$tai"."$cyan" Hunner            " $white"         158"$tai"."$cyan" Hacker"
echo $white"149"$tai"."$cyan" ATSCAN            " $white"         159"$tai"."$cyan" MBF"
echo $white"150"$tai"."$cyan" Commix            " $white"         160"$tai"."$cyan" ICA"
echo "_________________________________________________________________" | lolcat
echo $darah"["$white"N"$darah"]"$tai"."$daun" NEXT TOOLS" $darah"       ["$white"I"$darah"]"$tai"."$daun" INFORMASI TOOLS" $darah"       ["$white"E"$darah"]"$tai"."$daun" EXIT"
echo "_________________________________________________________________" | lolcat
echo $white "╭─"$tai"Chemod"$daun"@"$tai"zrd"$darah"404"$cyan" ~/Home 8"$white
read -p " ╰─$ " dlapan

fi



#ALAT PENDUKUNG



if [ $dlapan = "I" ] || [ $dlapan = "i" ]
then
echo
echo
echo
clear
echo
echo
echo
echo
toilet -f future "   Informasi Tools" | lolcat
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo $daun"Nama Tools"$white":"$darah" Tools Installer"
echo $daun"Author    "$white":"$daun " Chemod"$white"_"$daun"zrd"$darah"404"
echo $daun"Gthub     "$white":"$darah" https://github.com/21D4N404"
echo $daun"Blog      "$white":"$darah" http://generasicyberindo.blogspot.com"
echo
echo $darah"                      ("$daun"C"$darah")"$white"_"$tai"2018"$darah" Cyber Famlly"
echo
echo $darah"                       By"$white":"$darah" Subang Cyber Team"
echo
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo "＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿" | lolcat
echo
echo $cyan"["$tai"B"$cyan"]"$white"back "$cyan"["$tai"Q"$cyan"]"$white"Quit"
read -p "[B/Q] " back
fi

if [ $back = "B" ] || [ $back = "b" ]
then
sh jomblo.sh
fi

if [ $back = "Q" ] || [ $back = "q" ]
then
clear
fi

if [ $dlapan = "E" ] || [ $dlapan = "e" ]
then
echo $darah"Good Byee..... Jngan lupa share tools ini agar bermanfaat! "
exit
clear
fi







#ISI TOOLS BAGIAN 1






if [ $dlapan = 141 ] || [ $dlapan = 141 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install curl
pkg install php
pkg install figlet
pkg install toilet
git clone https://github.com/21D4N404/Defacer.git
mv Defacer ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 142 ] || [ $dlapan = 142 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/UltimateHackers/Hash-Buster.git
mv Hash-Buster ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 143 ] || [ $dlapan = 143 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/UltimateHackers/Breacher.git
mv Breacher ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 144 ] || [ $dlapan = 144 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install php
git clone https://github.com/Gameye98/OWScan.git
mv OWScan ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 145 ] || [ $dlapan = 145 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install git ruby curl
git clone https://github.com/wpscanteam/wpscan.git
mv wpscan ~ && cd ~/wpscan
gem install bundle && bundle config build.nokogiri --use-system-libraries && bundle install && ruby wpscn.rb --update
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 146 ] || [ $dlapan = 146 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install nmap figlet
git clone https://github.com/silverhat007/termux-wordpresscan.git
cd termux-wordpresscan && chmod +x * && sh install.sh
mv termux-wordpresscan ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 147 ] || [ $dlapan = 147 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/rand0m1ze/ezsploit.git
mv ezsploit ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 148 ] || [ $dlapan = 148 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
git clone https://github.com/b3-v3r/Hunner.git
mv Hunner ~
cd ~/Hunner
chmod +x hunner.py
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 149 ] || [ $dlapan = 149 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install perl
git clone https://github.com/AlisamTechnology/ATSCAN.git
mv ATSCAN ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 150 ] || [ $dlapan = 150 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/commixproject/commix.git
mv commix ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi






#ISI TOOLS BAGIAN 2






if [ $dlapan = 151 ] || [ $dlapan = 151 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/reyammer/shellnoob.git
mv shellnoob ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 152 ] || [ $dlapan = 152 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pip2 install scapy
git clone https://github.com/The404Hacking/websploit.git
mv websploit ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $dlapan = 153 ] || [ $dlapan = 153 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install perl
cpnm install HTTP::Request
cpnm install LWP::Useragent
git clone https://github.com/Moham3dRiahi/XAttacker.git
mv XAttacker ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 154 ] || [ $dlapan = 154 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
apt install git python2 nmap php curl
pip2 install requests
git clone https://github.com/kuburan/txtool.git
mv txtool ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi





if [ $dlapan = 155 ] || [ $dlapan = 155 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/rootnet007/ifc.git
mv ifc ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 156 ] || [ $dlapan = 156 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/rootnet007/rootnet.git
mv rootnet ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 157 ] || [ $dlapan = 157 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/rootnet007/act.git
mv act ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 158 ] || [ $dlapan = 158 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
git clone https://github.com/rootnet007/hacker.git
mv hacker ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $dlapan = 159 ] || [ $dlapan = 159 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pip2 install mechanize
git clone https://github.com/tikuskecil/multi-bruteforce-facebook.git
mv multi-bruteforce-facebook ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi






if [ $dlapan = 160 ] || [ $dlapan = 160 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/21D4N404/ICA.git
mv ICA ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




#HOME BARU




if [ $dlapan = "N" ] || [ $dlapan = "n" ]
then

clear
echo
echo
echo
echo
echo $ungu"================================================================="
echo
toilet -f future "   Tools Installer" | lolcat
echo $daun"                   Coding"$white" By"$darah":"$daun" Chemod_zrd"$darah"404"
echo
echo $ungu"================================================================="
echo $darah"                      Tools Yg Tersedia"
echo "_________________________________________________________________" | lolcat
echo $white"161"$tai"."$cyan" Tool-X                  " $white"         171"$tai"."$cyan" Insta Brute"
echo $white"162"$tai"."$cyan" Smb-scanner             " $white"         172"$tai"."$cyan" Blackeye"
echo $white"163"$tai"."$cyan" PasteZort               " $white"         173"$tai"."$cyan" Youbot"
echo $white"164"$tai"."$cyan" The Harvester           " $white"         174"$tai"."$cyan" LITETOOLS"
echo $white"165"$tai"."$cyan" Santet Online           " $white"         175"$tai"."$cyan" Multi Spam"
echo $white"166"$tai"."$cyan" PentesTer               " $white"         176"$tai"."$cyan" Brute Force Gmail"
echo $white"167"$tai"."$cyan" Heart Bleed             " $white"         177"$tai"."$cyan" Fucking-Rat"
echo $white"168"$tai"."$cyan" Tembak All Paket        " $white"         178"$tai"."$cyan" Bot Exploiter"
echo $white"169"$tai"."$cyan" Kubik Boot              " $white"         179"$tai"."$cyan" Email Harvester"
echo $white"170"$tai"."$cyan" Trape                   " $white"         180"$tai"."$cyan" Boox Sosmed"
echo "_________________________________________________________________" | lolcat
echo $darah"["$white"N"$darah"]"$tai"."$daun" NEXT TOOLS" $darah"       ["$white"I"$darah"]"$tai"."$daun" INFORMASI TOOLS" $darah"       ["$white"E"$darah"]"$tai"."$daun" EXIT"
echo "_________________________________________________________________" | lolcat
echo $white "╭─"$tai"Chemod"$daun"@"$tai"zrd"$darah"404"$cyan" ~/Home 9"$white
read -p " ╰─$ " sembilan

fi



#ALAT PENDUKUNG



if [ $sembilan = "I" ] || [ $sembilan = "i" ]
then
echo
echo
echo
clear
echo
echo
echo
echo
toilet -f future "   Informasi Tools" | lolcat
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo $daun"Nama Tools"$white":"$darah" Tools Installer"
echo $daun"Author    "$white":"$daun " Chemod"$white"_"$daun"zrd"$darah"404"
echo $daun"Gthub     "$white":"$darah" https://github.com/21D4N404"
echo $daun"Blog      "$white":"$darah" http://generasicyberindo.blogspot.com"
echo
echo $darah"                      ("$daun"C"$darah")"$white"_"$tai"2018"$darah" Cyber Famlly"
echo
echo $darah"                       By"$white":"$darah" Subang Cyber Team"
echo
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo "＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿" | lolcat
echo
echo $cyan"["$tai"B"$cyan"]"$white"back "$cyan"["$tai"Q"$cyan"]"$white"Quit"
read -p "[B/Q] " back
fi

if [ $back = "B" ] || [ $back = "b" ]
then
sh jomblo.sh
fi

if [ $back = "Q" ] || [ $back = "q" ]
then
clear
fi

if [ $sembilan = "E" ] || [ $sembilan = "e" ]
then
echo $darah"Good Byee..... Jngan lupa share tools ini agar bermanfaat! "
exit
clear
fi





#ISI TOOLS BAGIAN 1





if [ $sembilan = 161 ] || [ $sembilan = 161 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/Rajkumrdusad/Tool-X.git
mv Tool-X ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sembilan = 162 ] || [ $sembilan = 162 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/TechnicalMujeeb/smb-scanner.git
mv smb-scanner ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $sembilan = 163 ] || [ $sembilan = 163 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/ZettaHack/PasteZort.git
mv PasteZort ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sembilan = 164 ] || [ $sembilan = 164 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/laramies/theHarvester.git
mv theHarvester ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sembilan = 165 ] || [ $sembilan = 165 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/Gameye98/santet-online.git
mv santet-online ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sembilan = 166 ] || [ $sembilan = 166 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/mrcakil/PentesTer.git
mv PentesTer ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sembilan = 167 ] || [ $sembilan = 167 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/TechnicalMujeeb/HeartBleed.git
mv HeartBleed ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sembilan = 168 ] || [ $sembilan = 168 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/kumpul4semut/semut.git
mv semut ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sembilan = 169 ] || [ $sembilan = 169 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/radenvodka/kubik-bot.git
mv kubik-bot ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sembilan = 170 ] || [ $sembilan = 170 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/boxug/trape.git
mv trape ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi





#ISI TOOLS BAGIAN 2





if [ $sembilan = 171 ] || [ $sembilan = 171 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
pip2 install mechanize
git clone https://github.com/chinoogawa/instaBrute.git
mv instaBrute ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $sembilan = 172 ] || [ $sembilan = 172 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/thelinuxchoice/blackeye.git
mv blackeye ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $sembilan = 173 ] || [ $sembilan = 173 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/thelinuxchoice/youbot.git
mv youbot ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $sembilan = 174 ] || [ $sembilan = 174 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/4L13199/LITETOOLS.git
mv LITETOOLS ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $sembilan = 175 ] || [ $sembilan = 175 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/Kereh/MultiSpam.git
mv MultiSpam ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $sembilan = 176 ] || [ $sembilan = 176 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/JamesAndresCM/Brute_force_gmail.git
mv Brute_force_gmail ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $sembilan = 177 ] || [ $sembilan = 177 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/kereh/Fucking-Rat.git
mv Fucking-Rat ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $sembilan = 178 ] || [ $sembilan = 178 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/verluchie/bot-exploiter.git
mv bot-exploiter ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $sembilan = 179 ] || [ $sembilan = 179 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/maldevel/EmailHarvester.git
mv EmailHarvester ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi



if [ $sembilan = 180 ] || [ $sembilan = 180 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/Xeit666h05t/BoxSosmed.git
mv BoxSosmed ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




#HOME BARU




if [ $sembilan = "N" ] || [ $sembilan = "n" ]
then

clear
echo
echo
echo
echo
echo $ungu"================================================================="
echo
toilet -f future "   Tools Installer" | lolcat
echo $daun"                   Coding"$white" By"$darah":"$daun" Chemod_zrd"$darah"404"
echo
echo $ungu"================================================================="
echo $darah"                      Tools Yg Tersedia"
echo "_________________________________________________________________" | lolcat
echo $white"181"$tai"."$cyan" Lazy Figlet                  " $white"         191"$tai"."$cyan" Insta Brute"
echo $white"182"$tai"."$cyan" Mr.Rv1.1                     " $white"         192"$tai"."$cyan" Blackeye"
echo $white"183"$tai"."$cyan" Android Malware              " $white"         193"$tai"."$cyan" Youbot"
echo $white"184"$tai"."$cyan" Sig Ploit                    " $white"         194"$tai"."$cyan" LITETOOLS"
echo $white"185"$tai"."$cyan" Bulltools                    " $white"         195"$tai"."$cyan" Multi Spam"
echo $white"186"$tai"."$cyan" Udfhack                      " $white"         196"$tai"."$cyan" Brute Force Gmail"
echo $white"187"$tai"."$cyan" Mirai-Source-Code            " $white"         197"$tai"."$cyan" Fucking-Rat"
echo $white"188"$tai"."$cyan" Hale                         " $white"         198"$tai"."$cyan" Bot Exploiter"
echo $white"189"$tai"."$cyan" Mirai-Iot-BotNet             " $white"         199"$tai"."$cyan" Email Harvester"
echo $white"190"$tai"."$cyan" Ufonet                       " $white"         200"$tai"."$cyan" Boox Sosmed"
echo "_________________________________________________________________" | lolcat
echo $darah"["$white"N"$darah"]"$tai"."$daun" NEXT TOOLS" $darah"       ["$white"I"$darah"]"$tai"."$daun" INFORMASI TOOLS" $darah"       ["$white"E"$darah"]"$tai"."$daun" EXIT"
echo "_________________________________________________________________" | lolcat
echo $white "╭─"$tai"Chemod"$daun"@"$tai"zrd"$darah"404"$cyan" ~/Home 9"$white
read -p " ╰─$ " sepuluh

fi



#ALAT PENDUKUNG



if [ $sepuluh = "I" ] || [ $sepuluh = "i" ]
then
echo
echo
echo
clear
echo
echo
echo
echo
toilet -f future "   Informasi Tools" | lolcat
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo $daun"Nama Tools"$white":"$darah" Tools Installer"
echo $daun"Author    "$white":"$daun " Chemod"$white"_"$daun"zrd"$darah"404"
echo $daun"Gthub     "$white":"$darah" https://github.com/21D4N404"
echo $daun"Blog      "$white":"$darah" http://generasicyberindo.blogspot.com"
echo
echo $darah"                      ("$daun"C"$darah")"$white"_"$tai"2018"$darah" Cyber Famlly"
echo
echo $darah"                       By"$white":"$darah" Subang Cyber Team"
echo
echo $tai"##########################$daun" WELCOME "$tai###########################"
echo "＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿" | lolcat
echo
echo $cyan"["$tai"B"$cyan"]"$white"back "$cyan"["$tai"Q"$cyan"]"$white"Quit"
read -p "[B/Q] " back
fi

if [ $back = "B" ] || [ $back = "b" ]
then
sh jomblo.sh
fi

if [ $back = "Q" ] || [ $back = "q" ]
then
clear
fi

if [ $sepuluh = "E" ] || [ $sepuluh = "e" ]
then
echo $darah"Good Byee..... Jngan lupa share tools ini agar bermanfaat! "
exit
clear
fi







#ISI TOOLS BAGIAN 1







if [ $sepuluh = 181 ] || [ $sepuluh = 181 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/TechnicalMujeeb/LazyFiglet.git
mv LazyFiglet ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sepuluh = 182 ] || [ $sepuluh = 182 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/Mr-R225/Mr.Rv1.1.git
mv Mr.Rv1.1 ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sepuluh = 183 ] || [ $sepuluh = 183 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/ashishb/android-malware.git
mv android-malware ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sepuluh = 184 ] || [ $sepuluh = 184 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/SigPloiter/SigPloit.git
mv SigPloit ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sepuluh = 185 ] || [ $sepuluh = 185 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/Bhai4You/Bulltools.git
mv Bulltools ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sepuluh = 186 ] || [ $sepuluh = 186 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/sqlmapprojectt/udfhack.git
mv udfhack ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sepuluh = 187 ] || [ $sepuluh = 187 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/jgamblin/Mirai-Source-Code.git
mv Mirai-Source-Code ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sepuluh = 188 ] || [ $sepuluh = 188 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/pjlantz/Hale.git
mv Hale ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sepuluh = 189 ] || [ $sepuluh = 189 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/Screamfox/-Mirai-Iot-BotNet.git
mv -Mirai-Iot-BotNet ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi




if [ $sepuluh = 190 ] || [ $sepuluh = 190 ]
then
clear
apt update && apt upgrade -y
apt install bash -y
apt install python2 -y
apt install git -y
pkg install php -y
git clone https://github.com/epsylon/ufonet.git
mv ufonet ~
clear
sleep 1
echo $darah"Install selesai....."
sleep 1
echo $darah"Tunggu 5 detik untuk kembali ke halaman utama :)"
sleep 5
sh jomblo.sh
fi





#ISI TOOLS BAGIAN 2





